...
预检拦截时原提示写的是 `make dev-down`,但占用 50051/8080/8092/3000 的
`cocola-*-1` 容器是**全容器栈**(`make up-all` / `scripts/start.sh` + full.yml)起的,
`make dev-down` 只停 dev.yml 的 redis/pg/minio,停不掉这些 app 容器。
改为提示正确命令:`bash scripts/start.sh --down`。
