...
- **agent-runtime 控制面路由**：`COCOLA_AGENT_ROUTE=A` 启用
  ...
  NDJSON 事件流回传。Route B（`ClaudeAgentSDKProvider`）保留为最小 fallback，
  但其 MCP 转发缝合层已删除（见 2026-06-15 进展），原生工具不再经宿主转发。
...
- **范围抉择**：本次为外科式删缝合层。`ClaudeAgentSDKProvider` 本身**保留**为最小
  fallback（零沙箱 dev boot、llm-gateway JWT 透传契约测试仍依赖它），符合本 ADR
  §1 只点名删除「MCP 缝合层」的授权边界。收敛为 Route A 单路径（连带删 provider /
  `COCOLA_AGENT_ROUTE` 开关 / compose / run-stack）作为独立后续任务评估。
