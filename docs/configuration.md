...
| 类型           | 唯一来源                    | 生效方式       | 适用内容                                                                |
| -------------- | --------------------------- | -------------- | ----------------------------------------------------------------------- |
| 运行时业务配置 | Admin/Postgres              | 保存后热加载   | 模型、Prompt、MCP、Skill、Scheduler、Session 默认请求容量、Trace 保留期 |
...
Scheduler、Session 默认容量和 Trace 的同名环境变量只是无 DB override 时的默认值。
...
| Sandbox          | `COCOLA_SANDBOX_ADDR`、image、lease/reaper/heartbeat、LLM URL/model、egress                                                          |
