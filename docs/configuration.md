...
| Sandbox                | `COCOLA_SANDBOX_ADDR`、image、lease/reaper/heartbeat、LLM URL/model、egress、volume backend                                          |
...
`COCOLA_LLM_REDIS_URL`。鉴权密钥、Admin key、模型/配置加密密钥和 MinIO 是正式
运行必需项；缺失或启动健康检查失败时服务拒绝启动，不存在匿名、inline-only、
metadata-only 或 checkpoint-disabled 模式。源码开发由 `make dev` 准备依赖；正式
部署由 `cocola` CLI 生成并注入独立配置。
内存实现只用于测试，不是运行模式。
...
Image、LLM 路由和 refill interval 仍是启动配置。
...
| Warm Pool image / 路由 / refill interval | 重启                             | 创建参数与 ticker 在 sandbox-manager 启动时构造        |
