...
test("Admin Skill and MCP cards use HeroUI Switch controls", () => {
  ...
  assert.doesNotMatch(
    skillsSource,
    /variant="outline" isDisabled=\{working\} onPress=\{onToggle\}/,
  );
...
test("Admin catalog cards have the approved HeroUI Demo hover contract", () => {
  assert.match(skillsSource, /className="admin-skill-card min-h-64/);
...
test("Token Usage keeps range, trend, and user rows compact", () => {
  assert.match(
    tokenUsageSource,
    /className="admin-token-usage-range flex flex-wrap items-center gap-2"/,
  );
  ...
  assert.doesNotMatch(
    tokenUsageSource,
    /text-muted block truncate font-mono text-xs">\{user\.user_id\}/,
  );
