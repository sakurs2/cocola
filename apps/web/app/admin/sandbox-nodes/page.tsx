...
type OfflineTarget = { node: SandboxNode; busy: boolean };
...
export default function SandboxNodesPage() {
  ...
  const runAction = async (node: SandboxNode, action: "disable" | "restore" | "offline") => {
    setError("");
    ...
    try {
      const res = await fetch(
        `/api/admin/sandbox-nodes/${encodeURIComponent(node.name)}/${action}`,
        {
          ...
          body: action === "offline" ? JSON.stringify({ force: true }) : undefined,
        },
      );
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-6 py-6">
        ...
        <section className="rounded-lg border border-border bg-card">
          ...
          <div className="p-4">
            <code className="block overflow-x-auto rounded-md border border-border bg-muted px-3 py-2 font-mono text-xs text-foreground">
                        ...
                        <div className="flex justify-end gap-2">
                          {node.schedulable ? (
                            <Button
                              ...
                              onClick={() => void runAction(node, "disable")}
                            >
                            ...
                            <Button
                              ...
                              size="sm"
                              disabled={Boolean(actingNode)}
                              onClick={() => void runAction(node, "restore")}
                            >
                          ...
                          <Button
                            ...
                            onClick={() => setOfflineTarget({ node, busy: node.sandbox_pods > 0 })}
                          >
                            {offlining ? (
                              <LoaderCircle className="mr-2 size-4 animate-spin" />
                            ) : (
      ...
      {offlineTarget && (
        <OfflineDialog
          ...
          onConfirm={() => void runAction(offlineTarget.node, "offline")}
        />
      )}
    </main>
  );
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              This will cordon the node and request eviction for {target.node.sandbox_pods} sandbox
              pod{target.node.sandbox_pods === 1 ? "" : "s"}.
            </p>
          </div>
        </div>
        {target.busy && (
