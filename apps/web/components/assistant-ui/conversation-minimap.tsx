...
function formatTime(time: number | null, format: ReturnType<typeof useFormatter>): string {
  if (!time) return "";
  ...
  const hm = format.dateTime(d, { hour: "2-digit", minute: "2-digit" });
  if (sameDay) return hm;
  const md = format.dateTime(d, { month: "short", day: "numeric" });
  return `${md} ${hm}`;
}

export function ConversationMinimap() {
  const t = useTranslations("chat.minimap");
            ...
            <Tooltip.Root key={entry.id}>
              <Tooltip.Trigger asChild>
                <button
                  ...
                  aria-label={t("jump", { index: entry.index })}
