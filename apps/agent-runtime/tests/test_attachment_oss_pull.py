...
def _pwd_executor():
    return StaticSandboxExecutor(
        exec_handler=lambda sid, cmd: ExecOutcome(exit_code=0, stdout="/workspace/S9\n")
    )
...
async def test_key_only_attachment_is_pulled_and_written():
    ...
    assert writes == {"/workspace/S9/uploads/big.bin": b"\x89PNG\x00pulled"}
...
async def test_inline_attachment_is_not_pulled():
    ...
    assert fetcher.gets == []
    writes = {p: d for (_s, p, d) in ex.byte_writes}
    assert writes == {"/workspace/S9/uploads/a.txt": b"inline"}
