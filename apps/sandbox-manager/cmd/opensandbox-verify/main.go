...
//
// Stage 6 (-persist) proves the session storage mapping: it creates a sandbox,
// writes markers to /workspace and /home/cocola/.claude, destroys it, recreates
// one for the SAME session, and reads both markers back — verifying project
// files and hidden session-local Claude state survive a container destroy.
//
...
func main() {
	...
	// The whole point of volume mapping: data on the session workspace must
	// survive a sandbox being destroyed and a fresh one created for the same
	// session. Self-contained (own create/destroy pair) so it never disturbs the
	// main sandbox above. Real-server only.
	if *persist {
...
// destroy/recreate cycle. It uses a FIXED session id so both sandboxes map to
// the same cocola-session-* volume, then:
//
//	create A -> write markers to /workspace and /home/cocola/.claude ->
//	            destroy A -> create B (same session) -> read both markers back.
//
// Both must survive: project files and session-local Claude state. It also
// exercises the uid acceptance item by writing under /workspace as the sandbox
// user.
func verifyPersistence(ctx context.Context, p *opensandbox.Provider, run *runner, image string, cpu float64, mem int64) {
	...
	run.exec("6a. write .claude marker (session-local + uid write)", a.ID, provider.ExecRequest{
		Cmd: []string{"sh", "-c", "echo " + claudeMarker + " > " + claudeFile},
	}, 0)
	run.exec("6b. write session-workspace marker", a.ID, provider.ExecRequest{
	...
	run.exec("6c. .claude marker survived", b.ID, provider.ExecRequest{
		Cmd: []string{"sh", "-c", "cat " + claudeFile},
	}, 0)
	run.exec("6d. session marker survived", b.ID, provider.ExecRequest{
		Cmd: []string{"sh", "-c", "cat " + sessFile},
	}, 0)
	fmt.Printf("   (expect 6c=%q 6d=%q)\n", claudeMarker, sessMarker)
}
