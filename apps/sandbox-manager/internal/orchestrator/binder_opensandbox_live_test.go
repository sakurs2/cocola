...
func TestLive_OnDemandAllocation_OpenSandbox(t *testing.T) {
	...
	ev, err := p.Exec(ctx, sid, provider.ExecRequest{
		Cmd: []string{"sh", "-c", "echo dialogue-ok && ls -d /workspace /home/cocola/.claude /home/cocola/.codex /data/plugins"},
	})
	if err != nil {
	...
	for _, want := range []string{"dialogue-ok", "/workspace", "/home/cocola/.claude", "/home/cocola/.codex", "/data/plugins"} {
