...
| WP      | Warm pool 预热池:backend 无关引擎(idle 池 + 异步补池 + age-out,复用 rds.KV 的 Get-then-Del CAS,`SandboxProvider` 接口零改动)+ binder 接入 + 可选 `provider.Adopter` 缝口;空池/disabled/adopt 失败均静默降级 cold create。**ADR-0015:OpenSandbox 成为唯一主后端后,cocola 默认走按需冷启动分配;warm pool 降级为默认关闭的可选优化,当前主后端无 Adopter、开启亦只空转**(adopt-by-remount 在 OpenSandbox 运行中沙箱无热挂卷 API,见 ADR-0015/0012) | ✅(引擎,默认关闭) |
| GV      | gVisor(runsc)兼容性 spike:Node + Claude Code 在 `RuntimeClass=runsc` 下跑通 Route A 的 pre-prod 验收门;并承接 K8s warm-pool 节点镜像预拉(DaemonSet)与冷启动实测。Layer A/B 本机可做,Layer C(真集群 + gVisor 端到端)待目标集群 | ⏳   |
...
两种 provider 各自用所在平台的成熟原语落地,不引第三方守护进程:

| Provider | 强制机制 | 域名 allowlist |
| --- | --- | --- |
| Docker | `iptables` + `ipset` 默认 DROP + allowlist(复用 Anthropic Claude Code devcontainer init-firewall 模式);容器主进程 root 装防火墙,用户代码经 exec 钉非 root | best-effort 解析(getent) |
| K8s | 原生 `NetworkPolicy`:基线放行 DNS+集群内 gateway,CIDR/IP 作 ipBlock | 不支持,跳过并告警;需 DNS-aware CNI |

**域名级精确放行**:vanilla CNI 的 `NetworkPolicy` 只能匹配 IP/CIDR,域名条目会被
跳过并告警。要精确钉域名,需启用 DNS-aware CNI(如 Cilium 的 `CiliumNetworkPolicy`
`toFQDNs`)——已在 `deploy/helm/cocola-sandbox/values.yaml` 留好扩展点与示例,本项目
不强制引入。
