...
make up-all    # + llm-gateway（真实模型链路，Route A：大脑在沙箱内）+ 前端
```

> 端口约定：gateway BFF 与 llm-gateway 默认都监听 `8080`，编排脚本把
> llm-gateway 改钉到 `8081`（`COCOLA_LLM_PORT`）规避端口冲突；沙箱内大脑经
> `COCOLA_SANDBOX_LLM_BASE_URL` 回连它。跑真实模型（Route A）须导出
> `COCOLA_SANDBOX_ADDR` 指向一个 sandbox-manager，脚本会透传给 agent-runtime。
...
> `make up` dev 模式默认 bootstrap 管理员为 `admin` 或 `admin@cocola.local`，
