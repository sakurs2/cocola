...
| WP      | Warm pool 预热池:backend 无关引擎(idle 池 + 异步补池 + age-out,复用 rds.KV 的 Get-then-Del CAS,`SandboxProvider` 接口零改动)+ binder 接入 + 可选 `provider.Adopter` 缝口;空池/disabled/adopt 失败均静默降级 cold create。adopt-by-remount 在 PVC/bind-mount 卷模型上不可行(见 ADR-0012),K8s 落地改走节点镜像预拉,随下一里程碑实测 | ✅(引擎) |
